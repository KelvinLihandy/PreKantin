...
<div class="container-fluid min-vh-100 py-5" style="background-color: #C9E2FE;">
    <div class="container">
        ...
        <a href="{{ route('order.history') }}" class="btn btn-light fw-bold mb-4 shadow-sm text-primary">
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    ...
                    <div class="card-header bg-white p-4 border-bottom">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h4 class="fw-bold m-0 text-dark">Detail Pesanan</h4>
                                <p class="text-muted m-0 small">Order ID: #{{ $order->order_id }}</p>
                            </div>
                            <span class="badge bg-primary fs-6 px-3 py-2">{{ $order->status->name }}</span>
                        </div>
                    </div>

                    <div class="card-body p-4 bg-white">
                        <div class="mb-4 pb-3 border-bottom">
                            <h6 class="fw-bold text-muted text-uppercase small">Merchant</h6>
                            <h5 class="fw-bold text-primary">{{ $order->merchant->user->name }}</h5>
                            <p class="text-muted m-0 small">{{ $order->order_time->format('l, d F Y - H:i') }} WIB</p>
                        </div>

                        <h6 class="fw-bold text-muted text-uppercase small mb-3">Item Pesanan</h6>
                        
                        @foreach($order->orderItems as $item)
                        <div class="d-flex align-items-center mb-3">
                            <img src="{{ asset($item->menu_item->image) }}" 
                    ...
                    <div class="card-footer bg-light p-4 text-center">
                        <p class="small text-muted m-0">Terima kasih sudah memesan di PreKantin!</p>
                    </div>
                </div>
            </div>
        </div>
